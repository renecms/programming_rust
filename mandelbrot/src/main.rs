...
use image::ColorType;
use num::Complex;
use std::{env, fs::File, process::exit, str::FromStr};
...
/// it took for `c` to leave the circle of the radius 2 centered on the origin.
...
/// ASCII character.
///
/// If `s` has proper form, return `Some<(x, y)>`. If it doesn't parse
